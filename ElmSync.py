...
def ElmSync(time, nshot, syncSetting=None,
    ...
    if plot:
        ...
        ipolsola = sf('Ipolsola')
        time_ipolsola = ipolsola.time
        ipolsola = ipolsola.data
        f = plt.figure()
...
def ElmExtract(time, nshot, syncSetting=None, plot=False, preFac=0.1, postFac=0.2, Experiment='AUGD'):
    ...
    for i in np.arange(np.size(t_beg_ELM)):
        ...
        ind = np.squeeze(np.where((time >= t_beg_ELM[i] - t_beg_range) & (time <= t_end_ELM[i] + t_end_range)))
