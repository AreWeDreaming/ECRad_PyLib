...
if("SuSE-12" in platform.platform()):
    libkk = np.ctypeslib.load_library("libkk8x", "/afs/ipp-garching.mpg.de/aug/ads/lib64/@sys/")
else:
...
# Numpy output
        output = kkhelp()
        output.r = np.zeros((nAngle, nr))
        output.z = np.zeros((nAngle, nr))
        ...
        if nDimAngle != 2:
            for i, an in enumerate(np.ravel(angle)):
                ...
                output.r[i, :] = np.array(c_rn[0:nr])
                output.z[i, :] = np.array(c_zn[0:nr])
            ...
            for j in (np.arange(nr)):
                for i in (np.arange(nAngle)):
                    ...
                    output.r[i, j] = np.array(c_rn[0])
                    output.z[i, j] = np.array(c_zn[0])
...
# Numpy output
        output.q = np.array(q[0:nr])
        output.fpf = np.array(fpf[0:nr])
        output.rho_p = np.array(rhop[0:nr])
