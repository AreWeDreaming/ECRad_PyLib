...
def ECRad_plot():
    ...
    fig = plt.figure(figsize=(12.0, 8.5), tight_layout=False)
    pc_obj = plotting_core(fig)
    pc_obj.ECRad_plot(shot, time, folder)
    plt.show()

# tb_plot()

def comp_tb_ECRad_input():
    R_mat = np.loadtxt("/afs/ipp-garching.mpg.de/home/s/sdenk/F90/ECRad_Model_new/R_test")
    z_mat = np.loadtxt("/afs/ipp-garching.mpg.de/home/s/sdenk/F90/ECRad_Model_new/z_test")
    rhop = np.loadtxt("/afs/ipp-garching.mpg.de/home/s/sdenk/F90/ECRad_Model_new/rhop_test")
    ECRad_ray = np.loadtxt("/afs/ipp-garching.mpg.de/home/s/sdenk/F90/ECRad_Model_new/k_out_ECRad")
    tb_ray = np.loadtxt("/afs/ipp-garching.mpg.de/home/s/sdenk/F90//torbeam/k_out")
    svec = np.loadtxt("/ptmp1/work/sdenk/ECRad/ECRad_data/chdata050.dat")
    fig = plt.figure(figsize=(12.0, 8.5), tight_layout=False)
    ...
    N_ECRad = func_N(ECRad_ray.T[7], ECRad_ray.T[8], ECRad_ray.T[9], -1.0)
    pc_obj.tb_check_plot(tb_ray, ECRad_ray, R_mat, z_mat, rhop, N_tb, N_ECRad)
    plt.show()
