...
def plt_eq_tor(ax, nshot, time, rhos=np.linspace(0., 1., num=10), linewidth=1, linecolor='k', linestyleSep='solid', linestyleFlux='dashed', diag='EQI', exp='AUGD'):
    ...
    if np.size(out1.pf) == 1:
        ...
        for pf in out1.pf:
            ...
            if rhos[jrho] == 1.:
                ax.add_patch(Circle((0., 0.), np.max(out2.r_surf), fill=False, edgecolor='b', linewidth=3))
            elif rhos[jrho] == 0.:
                ax.add_patch(Circle((0., 0.), np.max(out2.r_surf), fill=False, edgecolor='r'))
            else:
