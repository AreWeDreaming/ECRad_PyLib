...
def CECE_workflow(working_dir, Results_filename, time):
    ...
    run_ECE_TORBEAM_ECRad_Scenario(working_dir, Results_filename, time, logfile=logfile)
...
def get_BPD_width(Results, itime=0, logfile=None):
    ...
    for ich in range(len(Results.Scenario["diagnostic"]["f"][itime])):
        ...
        R_in_spl = InterpolatedUnivariateSpline(Results["ray"]["s"][itime][ich][0][0], 
                                                Results["ray"]["rhop"][itime][ich][0][0]
                                                 - fitted_params[1] + fitted_params[2]/2.0)
        R_out_spl = InterpolatedUnivariateSpline(Results["ray"]["s"][itime][ich][0][0], 
                                                Results["ray"]["rhop"][itime][ich][0][0]
                                                 - fitted_params[1] - fitted_params[2]/2.0)
...
def run_ECE_TORBEAM_ECRad_Scenario(working_dir, ECRad_Scenario_file, time, logfile=None):
    ...
    TB_results = torbeam_interface(working_dir, Scenario["shot"], time, itime, 
            plasma_dict, eq_slice, ece_launch, logfile=logfile)
...
def torbeam_interface(working_dir, shot, time, itime, plasma_dict, eq_slice, ece_launch, logfile=None):
    cece_launches = []
    ...
    for ich in range(len(ece_launch["f"][itime])):
        ...
        try:
            ...
            root_spl = InterpolatedUnivariateSpline(s[mask], 
                    2.0 * B_tot_spl(R_center[mask]/1.e2, z_center[mask]/1.e2, grid=False) * cnst.e / cnst.m_e
        ...
        x, z = plot1DECE(freq=ece_launch["f"][itime][ich]/1.e9, dtoECE=60, project='poloidal', doPlot=False)
        ...
        s = np.zeros(R_center.shape)
        s[1:] = np.sqrt((R_center[1:] - R_center[:-1])**2 + 
                        (z_center[1:] - z_center[:-1])**2)
        s = np.cumsum(s)
        root_spl = InterpolatedUnivariateSpline(s[mask], 
            2.0 * B_tot_spl(R_center[mask], z_center[mask], grid=False) * cnst.e / cnst.m_e
            - cece_launches[0].f * 2.0 * np.pi )
        roots = root_spl.roots()
        s_res = np.max(roots)
        waist_spl = InterpolatedUnivariateSpline(s, waist)
        if(logfile is not None):
            logfile.write("Vacuum width channel {0:d}: {1:1.3f} cm\n".format(ich + 1, waist_spl(s_res) * 100.0))
        else:
            print("Vacuum width channel {0:d}: {1:1.3f} cm\n".format(ich + 1, waist_spl(s_res) / 10.0))
