...
# Input
    ...
    def kkeqpfm(self,nshot, time, exp=exp_eq, diag=dia_eq, ed=0,  m=65, n=129):
        o_m = m
        o_n = n
        c_ed    = ct.c_long(ed)
        _ed     = ct.byref(c_ed)
        ...
        #   &mdim, &m, &n, Ri, zj, pfm,
        #   long, long);
        libkk.kkeqpfm_(_err, c_exp, c_dia, _shot, _ed, _t,
                        _mdim, _m, _n, _Ri, _zj, _pfm)
        output     = kkhelp()
        output.ed  = c_ed.value
        output.err = c_err.value
        # For some reason m and n are one smaller than they should be....
        output.R = np.frombuffer(Ri, np.float32)[:m.value + 1]
        output.z = np.frombuffer(zj, np.float32)[:n.value + 1]
        output.Psi = np.frombuffer(pfm, np.float32).reshape(o_n, o_m)[:n.value + 1, :m.value + 1]
        output.Psi = output.Psi.T
        return output
...
# Float output
        ...
        output.Rspx_2 = c_rpfx[3]
        output.zspx_2 = c_zpfx[3]
        output.psiaxis = c_pfx[0]
        output.psispx = c_pfx[1]
        output.psilim = c_pfx[2]
        return output
...
# Output
        ...
        output.err = c_err.value
# Numpy output
        output.r = np.array(c_rn[0:nr])
        output.z = np.array(c_zn[0:nr])
