...
# import gc
    # gc.collect()
dd = dd.shotfile()
# TODO udlět že když to chce podruhé ten samý časový bod tak ho to bezme už z toho co je  je spočtené!
# TODO nečítat jen ty  úseky co poteřbuji, ale na PFM to nefunguje!! blbě seřazená v paměti
...
class equ_map:
    ...
    def kkrhopto(self, rho, tin, shot, diag='EQH', exp='AUGD', ed=0, integ_time=0, rho_lbl='rho_pol', extrapolate=False):
        """Fast and smooth!  mapping from rho_pol -> rho_tor or rho_tor -> rho_pol
        ...
        """
        if not rho_lbl in ['rho_pol', 'rho_tor']:
        ...
        self.tvec = self.tvec[:PFL.shape[1]]
        ...
        # print tin, np.min(tin), np.max(tin)

        ind = (self.tvec >= np.min(tin) - dt - integ_time) & (self.tvec <= np.max(tin) + dt + integ_time)

        ikCAT = dd.GetSignalGroup('ikCAT')

        ikCAT2 = np.array([2, 0, 3, 1])[ikCAT - 1]
        ...
        sep = PFxx[ikCAT2, np.arange(len(mag))]


        sep2 = np.min((PFL[np.all(TFLx == 0, 1), :]), 0)  # best estimate of sep in rho_tor
        mag2 = np.max((PFL), 0)
        ...
        for jt, t in enumerate(tin):
            ...
            if i0 != i:  # already calculated in the previous step
                ...
                sort_wh = np.argsort(-PFL[:, i] * orientation)
                ind = TFLx[sort_wh, i] != 0  # get rid of the point out of the separatrix
                ind[0] = True
                sort_wh = sort_wh[ind]

                mg = np.interp(-mag[i] * orientation, -PFL[sort_wh, i] * orientation, TFLx[sort_wh, i])
                sp = np.interp(-sep[i] * orientation, -PFL[sort_wh, i] * orientation, TFLx[sort_wh, i])
                # sp = -np.max(abs(TFLx[:,i])) #best estimate of sep in rho_tor
                if abs(sp - mg) < 1e-4:     continue
                ...
                pfl[pfl > 1] = 1  # round errors
                tfl[tfl > 1] = 1  # round errors


                pfl, tfl = np.r_[0, np.sqrt(pfl), 1], np.r_[0, np.sqrt(tfl), 1]
                try:
                    if rho_lbl == 'rho_pol':
                        sortind = np.unique(pfl, return_index=True)[1]
                        w = np.ones_like(sortind)
                        w[[0, -1]] = 1e3
                        s = UnivariateSpline(pfl[sortind], tfl[sortind], w=w, k=4, s=2e-4, bbox=[0, 1])
                    elif rho_lbl == 'rho_tor':
                        sortind = np.unique(tfl, return_index=True)[1]
                        s = UnivariateSpline(tfl[sortind], pfl[sortind], k=3, s=9e-4, bbox=[0, 1])
                    ...
                    continue
                # plt.plot(pfl[sortind],tfl[sortind],lw=.3)
                # plt.plot(np.linspace(0,1,200),(s(np.linspace(0,1,200))),'--')
            # plt.plot(x,y,'--',lw=.3)
            i0 = i
            ...
            if not extrapolate:
                rho_out[jt, rho[jt, :] >= 1] = 1


        # plt.plot(rho.T, rho_out.T)
        ...
        rhotor2 = np.zeros_like(rho_out)
        for jt, t in enumerate(tin):
            pto = kk.KK().kkrhopto(shot, t, rho[jt, :], ed=ed, diag=diag)
            rhotor2[jt, :] = pto.rho_t
            ed = pto.ed
        plt.title('kkrhopto')
        plt.plot(rhotor2, 'r', linewidth=0.2)
        plt.plot(rho_out, 'b--', linewidth=0.2)
        plt.show()



    def kkrhoPVo(self, shot, tin, rho, diag='EQH', exp='AUGD', ed=0, rho_lbl='rho_pol', strahl_def=True, extrapolate=None):
    ...
    def kkrzbrzt(self, shot, tin, rin, zin, exp='AUGD', diag='EQH', ed=0, dR=0, dZ=0):
        ...
        for jt in xrange(ntin):
            ...
            interpBr[ind, :] = map_coordinates(Br, idx, mode='nearest', order=2, prefilter=True)
            interpBz[ind, :] = map_coordinates(Bz, idx, mode='nearest', order=2, prefilter=True)
            tin[ind] = np.nan
    ...
    def kkrzptfn(self, shot, tin, rin, zin, exp='AUGD', diag='EQH', ed=0, rho_lbl='rho_pol', integ_time=0, dR=0, dZ=0, extrapolate=False):
        """Equilibrium mapping routine, map from R,Z -> rho (pol,tor,r_V,...) , fast for large number of points
        ...
        # print ixti.shape, self.ixti.shape, not hasattr(self,'ixti') or not stat
        ...
        # print index.shape, ind.shape, tvec_.shape,ixti.shape
        index = index[ind]
        index -= index[0]
        nt = index[-1] + 1
        ixti = ixti[ind]
        ...
        # TFLx = np.zeros((93,nt))
        # TFLx[:,index] = self.TFLx[:,ixti]

        # PFL = np.zeros((93,nt))
        # PFL[:,index] = self.PFL[:,ixti]
        ...
        # print 'ikCAT',ikCAT,ikCAT2,orientation, tin
        sep = PFxx[ikCAT, np.arange(len(mag))]
        ...
        if dR != 0 or dZ != 0:
            if dR != 0:  rin = rin - dR * (1 - rho)
            if dZ != 0: zin = zin - dZ * (1 - rho)
            coords = np.array((rin, zin, tin_))
            idx = (coords - offset[:, None, None]) / scaling[:, None, None]
            map_coordinates(rp, idx, mode='nearest', order=2, prefilter=True, output=rho)
            rho[rho < 0] = 0
        ...
        t = time.time()
        if rho_lbl == 'rho_tor':
            rho = self.kkrhopto(rho, tin_, shot, diag, exp, ed, integ_time=integ_time, extrapolate=extrapolate)
        if rho_lbl == 'r_V':
            rho = self.kkrhoPVo(shot, tin, rho, diag, exp, ed, rho_lbl='rho_pol', strahl_def=False, extrapolate=extrapolate)
        if rho_lbl == 'r_V_strahl':
            rho = self.kkrhoPVo(shot, tin, rho, diag, exp, ed, rho_lbl='rho_pol', strahl_def=True, extrapolate=extrapolate)
        ...
        for j in range(ntin) :
            ptfn = kk.KK().kkrzptfn(int(shot), tin[j], rin[j, :], zin[j, :], exp=exp, diag=diag, ed=ed)
            rhop2[j, :] = ptfn.rho_p
            rhot2[j, :] = ptfn.rho_t
            ed = ptfn.ed
        # ptfn=kk.KK().kkrzptfn(int(shot),tin,rin[0,:],zin[0,:],exp=exp,diag=diag,ed=ed)
        # rhop2 = ptfn.rho_p
        # rhot2 = ptfn.rho_t
        ...
        # print tin.shape, rho.shape
        ax.plot(tin, rho, 'r', linewidth=0.2)


        # ax.plot(rho.T,'r',linewidth = 0.2)


        if rho_lbl == 'rho_pol':
            # print tin.shape, rhop2.shape, rho.shape
            ax.plot(tin, rhop2, 'b--', linewidth=0.2)
            # ax.plot( rhop2.T,'b--',linewidth = 0.2)
            # return rhop2
            ...
            # ax.plot( rhot2.T,'b--',linewidth = 0.2)
            # return rhot2
    ...
    def kkeqpsp_fast(self, shot, tin, rhoin, diag='EQH', exp='AUGD', ed=0, rho_lbl='rho_pol', dR=0, dZ=0):
        # fast elm corrected magnetic surfaces

        tp = kk.KK().kkeqtp(shot, exp=exp, diag=diag, ed=ed)


        ed = tp.ed

        near_interp = interp1d(tp.time, tp.time, kind='nearest')
        tin_near = np.unique(near_interp(tin))



        contours_slow = self.kkeqpsp(shot, tin_near, rhoin, diag=diag, exp=exp, ed=ed, rho_lbl=rho_lbl, dR=dR, dZ=dZ)
        ...
        if dd.Open(diag, shot, experiment=exp, edition=ed):
            tvec_slow = dd.GetSignal('time')
            # ind = slice(np.where(tvec_slow==tin_near[0])[0],np.where(tvec_slow==tin_near[-1])[0])

            Rmag_slow = dd.GetSignalGroup('SSQ')[14, :len(tvec_slow)]
            Zmag_slow = dd.GetSignalGroup('SSQ')[15, :len(tvec_slow)]
            ahor_slow = dd.GetSignalGroup('SSQ')[ 6, :len(tvec_slow)]
            bver_slow = dd.GetSignalGroup('SSQ')[35, :len(tvec_slow)]

            # tvec_slow = tvec_slow[ind]
            dd.Close()



        if dd.Open('FPG', shot, experiment=exp, edition=ed):
            tvec_fast = dd.GetTimebase('TIMEF')
            # ind = slice(np.where(tvec_fast<=tin.min()-0.1)[0][-1],np.where(tvec_fast>=tin.max())[0][0])

            Rmag_fast = dd.GetSignal('Rmag')
            Zmag_fast = dd.GetSignal('Zmag')
            ahor_fast = dd.GetSignal('ahor')
            bver_fast = dd.GetSignal('bver')
            # tvec_fast = tvec_fast[ind]
        ...
        from  equ.new_equ import  ElmCorrection


        if dd.Open('ELM', shot):

            tELM = dd.GetTimebase('tELM')
        ...
        tvec, Zmag_fast = ElmCorrection(tvec_fast, Zmag_fast, tvec_slow, Zmag_slow, tELM, dt_ELM, nsmooth=11, mode='replace')
        tvec, Rmag_fast = ElmCorrection(tvec_fast, Rmag_fast, tvec_slow, Rmag_slow, tELM, dt_ELM, nsmooth=11, mode='averadge')

        tvec, bver_fast = ElmCorrection(tvec_fast, bver_fast, tvec_slow, bver_slow, tELM, dt_ELM, nsmooth=11, mode='replace')
        tvec, ahor_fast = ElmCorrection(tvec_fast, ahor_fast, tvec_slow, ahor_slow, tELM, dt_ELM, nsmooth=11, mode='replace')


        Rmag_fast = np.interp(tin, tvec, Rmag_fast)
        Zmag_fast = np.interp(tin, tvec, Zmag_fast)
        ahor_fast = np.interp(tin, tvec, ahor_fast)
        bver_fast = np.interp(tin, tvec, bver_fast)


        Rmag_slow = np.interp(tin_near, tvec_slow, Rmag_slow)
        Zmag_slow = np.interp(tin_near, tvec_slow, Zmag_slow)
        ahor_slow = np.interp(tin_near, tvec_slow, ahor_slow)
        bver_slow = np.interp(tin_near, tvec_slow, bver_slow)





        # change shape of the mag. surfaces according to fast signals from FPG
        ...
        for ic, cs in enumerate(contours_slow):
            for r in cs:
                r[:, 0] -= Rmag_slow[ic]
                r[:, 0] /= ahor_slow[ic]
                r[:, 1] -= Zmag_slow[ic]
        ...
        for it, t in enumerate(tin):
            i = np.argmin(abs(t - tin_near))

            countours = deepcopy(contours_slow[i])
            ...
            for r in countours:
                ...
                r[:, 1] *= bver_fast[it]
                r[:, 1] += Zmag_fast[it]
                # print ahor_fast[it],bver_fast[it],Zmag_fast[it],Rmag_fast[it]
                # plt.plot(r[:,0], r[:,1])

            contours_final.append(countours)
            # plt.show()
    ...
    def kkeqpsp(self, shot, tin, rhoin, diag='EQH', exp='AUGD', ed=0, rho_lbl='rho_pol', dR=0, dZ=0):
        ...
        ikCAT = np.int_(dd.GetSignalGroup('ikCAT'))
        ...
        for it, t in enumerate(tin):
            ...
            try:
                ...
                print 'cntr.Cntr', R.shape, Z.shape, PFM[:nr, :nz, i1].T.shape, PFM.shape
                raise e
        ...
        for it, t in enumerate(tin):
            # print t
            # print tvec.shape, sep.shape, mag.shape
            jt = np.argmin(abs(tvec - t))
            Flux = rhoin[it, :] ** 2 * (sep[jt] - mag[jt]) + mag[jt]
            rho_contours = []
        ...
        plt.xlim(Rmesh.min(), Rmesh.max())
        plt.ylim(Zmesh.min(), Zmesh.max())
...
def main():
    ...
    diag = 'EQH'
    # exp='AUGD'
    rin = np.arange(-nr, nr) * .5 / nr + 1.70
    zin = np.linspace(-0.5, 0.6, nr * 2)
    rin = zin * 0 + 1.781
    tin = np.linspace(3, 3.15, 100)
    # tin = 5
    # tin = np.atleast_1d(tin)
    ...
    rhopol = np.linspace(0.01, 0.99, 10)
    ...
    zin = np.linspace(-0.5, 0.6, 30)
    rin = zin * 0 + 1.781
    rin = np.arange(-nr, nr) * .5 / nr + 1.70
    zin = rin * 0

    rho = kk.kkrzptfn(shot, tin, rin, zin, rho_lbl='rho_tor', integ_time=0, dR=0.00, dZ=0.00, diag='EQH')
    # #print rho.shape
    # print rho
    ...
    # rho = kk.kkrhoPVo(shot,tin,rho,diag='EQI', exp='AUGD', ed=0,rho_lbl='rho_tor')
    # plot(rho.T)
    # show()
    # exit()
    ...
    kk.kkeqpsp(shot, np.linspace(3, 4, 10), rhopol, diag='EQH', exp='AUGD', ed=0, rho_lbl='rho_pol')
